...
import com.ldl.module.imkit.util.QDQQFaceManager;
...
//        }
        ...
        for (QQFace qqFace : qqFaceList) {
            EmojiBean bean = new EmojiBean();
            bean.setId(qqFace.getName());
            bean.setUnicodeInt(qqFace.getRes());
            emojiBeanList.add(bean);
        }
    ...
    public static String CopySqliteFileFromRawToDatabases(String SqliteFileName) throws IOException {
        ...
        //复制的话这里需要换成自己项目的包名
        File dir = new File("data/data/" + AppUtils.getAppPackageName() + "/databases");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                inputStream = Utils.getApp().getClass().getClassLoader().getResourceAsStream("assets/" + SqliteFileName);
