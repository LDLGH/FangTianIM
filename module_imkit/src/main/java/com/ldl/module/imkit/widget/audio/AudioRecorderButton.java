...
public class AudioRecorderButton extends AppCompatButton implements AudioManage.AudioStateListener {
    ...
    private final int[] arrayImageId = new int[]{R.mipmap.ic_record_volume_01, R.mipmap.ic_record_volume_02, R.mipmap.ic_record_volume_03
            , R.mipmap.ic_record_volume_04, R.mipmap.ic_record_volume_05, R.mipmap.ic_record_volume_06};
    ...
    public AudioRecorderButton(final Context context, AttributeSet attrs) {
        ...
        //音频文件保存路径
        String dir = getContext().getFilesDir() + "/" + "voice_";
        //获取音频管理器
    ...
    private void changeState(int state) {
        if (mCurState != state) {
            ...
            switch (state) {
                case STATE_NORMAL:
                    setBackgroundResource(R.drawable.shape_session_btn_voice);
                    setText(getContext().getString(R.string.hold_to_speak));
                    break;

                case STATE_RECORDERING:
                    setBackgroundResource(R.drawable.shape_session_btn_voice);
                    setText(getContext().getString(R.string.release_send));
                    if (isRecordering) {
                        // 更新Dialog.recording()
                ...
                case STATE_WANT_TO_CALCEL:
                    setBackgroundResource(R.drawable.shape_session_btn_voice);
                    setText(getContext().getString(R.string.cancel_sending));
                    // 更新Dialog.wantCancel()
